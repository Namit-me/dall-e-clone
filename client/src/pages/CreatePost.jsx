...
const CreatePosts = () => {
  ...
  const generateImage = async() => { 
    if(form.prompt){
      try {
        ...
        const response = await fetch("https://dall-e-ijxx.onrender.com/api/v1/dalle", {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async(e) => {
    ...
    if(form.prompt && form.photo){
      ...
      try {
        const response = await fetch("https://dall-e-ijxx.onrender.com/api/v1/post", {
          method: 'POST',
          headers: {
            'Content-Type':'application/json',
          },
  ...
  return (
   <section className='mx-auto max-w-7xl'>
      ...
      <form className='max-w-3xl mt-16' onSubmit={handleSubmit}>
        ...
        <div className='mt-10'>
          <p className='mt-2 text-[#666e75] text-[14px]'>Once you have created the image you want, you can share it with others inthe Community</p>
          <button
            type='submit'
            className='mt-3 text-white bg-[#6468ff]
